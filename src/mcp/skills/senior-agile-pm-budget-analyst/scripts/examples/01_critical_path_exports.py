...
def export_to_markdown(result: dict) -> str:
    ...
    lines.append(
        f"- **Project Duration**: {result['project_duration']} {result['unit']}"
    )
    lines.append(f"- **Critical Path**: {' → '.join(result['critical_path'])}")
    lines.append(
        f"- **Critical Tasks**: {result['statistics']['critical_activities_count']}"
    )
    ...
    crit_rows = [
        [t["id"], t["name"], f"{t['duration']}{result['unit']}", t["ES"], t["EF"]]
    ...
    stats_rows = [
        ...
        [
            "Average Slack (non-critical)",
            f"{stats['average_slack']:.2f} {result['unit']}",
        ],
...
def export_to_mermaid_flowchart(result: dict) -> str:
    ...
    for act in result["activities"]:
        for pred in act.get("predecessors", []):
            ...
            if act["critical"]:
                label = "Critical"
            edges.append({"from": pred, "to": act["id"], "label": label})
...
def export_to_html_report(result: dict) -> str:
    ...
    for act in result["activities"]:
        crit_marker = (
            '<span class="critical">✓ CRITICAL</span>' if act["critical"] else ""
        )
        rows.append(
            [
                act["id"],
    ...
    if result["bottlenecks"]:
        ...
        bottleneck_content += "</ul>"
        sections.append(
            {"heading": "⚠️ Bottlenecks & Risks", "content": bottleneck_content}
        )
