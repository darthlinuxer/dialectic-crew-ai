...
class ProjectAnalysisReport:
    ...
    def run_budget_analysis(
        self, story_points: int, team: list, fixed_costs: float
    ) -> dict:
    ...
    def run_poker_planning_analysis(self, stories: list) -> dict:
        ...
        if "error" not in velocity_result:
            print(
                f"   ✅ Average Velocity: {velocity_result['average_velocity']:.1f} points/sprint"
            )
    ...
    def generate_markdown_report(self) -> str:
        ...
        if "budget" in self.analyses:
            ...
            lines.append(
                f"- **Total Budget**: ${budget['summary']['total_budget']:,.2f}"
            )
            lines.append(
                f"- **Sprints Required**: {budget['project_metrics']['total_sprints']}"
            )
        ...
        if "poker_planning" in self.analyses:
            ...
            if "error" not in pp["velocity"]:
                lines.append(
                    f"- **Team Velocity**: {pp['velocity']['average_velocity']:.1f} points/sprint"
                )
    ...
    def generate_mermaid_diagrams(self) -> str:
        ...
        if "budget" in self.analyses:
            ...
            data = {
                role: info["cost"]
                for role, info in budget["breakdown"]["by_role"].items()
            }
    ...
    def generate_html_report(self) -> str:
        ...
        if "critical_path" in self.analyses:
            cpm = self.analyses["critical_path"]
            summary_content += (
                f"<li><strong>Duration:</strong> {cpm['project_duration']} days</li>"
            )
