...
def export_to_markdown(result: dict) -> str:
    ...
    lines.append(
        f"- **Overhead**: ${summary['overhead']:,.2f} ({summary['overhead_percentage']:.0f}%)"
    )
    lines.append(f"- **Fixed Costs**: ${summary['fixed_costs']:,.2f}")
    lines.append(
        f"- **Contingency**: ${summary['contingency']:,.2f} ({summary['contingency_percentage']:.0f}%)"
    )
    lines.append("")
    ...
    scenario_rows = [
        [
            "Optimistic (-15%)",
            f"${scenarios['optimistic']:,.2f}",
            f"-${summary['total_budget'] - scenarios['optimistic']:,.2f}",
        ],
        ["Realistic (Baseline)", f"${scenarios['realistic']:,.2f}", "$0.00"],
        [
            "Pessimistic (+25%)",
            f"${scenarios['pessimistic']:,.2f}",
            f"+${scenarios['pessimistic'] - summary['total_budget']:,.2f}",
        ],
    ]
    ...
    for role_name, role_data in breakdown.items():
        pct = (
            (role_data["cost"] / summary["base_cost"] * 100)
            if summary["base_cost"] > 0
            else 0
        )
        role_rows.append(
            [
    ...
    sprint_rows = [
        [f"Sprint {s['sprint_number']}", s["story_points"], f"${s['cost']:,.2f}"]
...
def main():
    ...
    team = [
        ...
        TeamMember(
            role="Senior Developer", count=2, hourly_rate=100, dedication_percent=100
        ),
