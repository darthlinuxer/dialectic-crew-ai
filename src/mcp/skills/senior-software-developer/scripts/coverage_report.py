        ...
        try:
            ...
            if coverage_file.exists():
                ...
                for cls in root.findall(".//class"):
                    filename = cls.attrib.get("filename", "")
                    line_coverage = float(cls.attrib.get("line-rate", 0)) * 100
                    files[filename] = {"line_coverage": line_coverage}
        ...
        try:
            ...
            if coverage_file.exists():
                ...
                for filepath, file_data in data.items():
                    if filepath != "total":
                        files[filepath] = {
                            ...
                            "branch_coverage": file_data.get("branches", {}).get(
                                "pct", 0
                            ),
                            "function_coverage": file_data.get("functions", {}).get(
                                "pct", 0
                            ),
                            "statement_coverage": file_data.get("statements", {}).get(
                                "pct", 0
                            ),
                        }
                ...
                return {
                    ...
                    "overall_function_coverage": total.get("functions", {}).get(
                        "pct", 0
                    ),
                    "overall_statement_coverage": total.get("statements", {}).get(
                        "pct", 0
                    ),
                    "files": files,
                }
    ...
    def generate_report(self):
        ...
        for language, data in results.items():
            ...
            if "overall_function_coverage" in data:
                print(
                    f"  Function Coverage: {data.get('overall_function_coverage', 0):.2f}%"
                )
            if "overall_statement_coverage" in data:
                print(
                    f"  Statement Coverage: {data.get('overall_statement_coverage', 0):.2f}%"
                )
