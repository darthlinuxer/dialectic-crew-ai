...
class QualityChecker:
    ...
    def check_python(self):
        ...
        print("Running black...")
        success, output = self.run_command(["black", "--check", "--diff", "."], "black")
        ...
        success, output = self.run_command(["flake8", "."], "flake8")
        self.results.append(("flake8 (linter)", success, output))

        # mypy - Type checking
        print("Running mypy...")
        success, output = self.run_command(["mypy", "."], "mypy")
        self.results.append(("mypy (type checker)", success, output))

        # pylint - Advanced linting
        print("Running pylint...")
        success, output = self.run_command(["pylint", "src/"], "pylint")
        self.results.append(("pylint (linter)", success, output))

        # pytest - Tests
        print("Running pytest...")
        success, output = self.run_command(
            ["pytest", "--cov=src", "--cov-report=term", "-v"], "pytest"
        )
    ...
    def check_csharp(self):
        ...
        success, output = self.run_command(
            ["dotnet", "build", "/warnaserror"], "dotnet build"
        )
        ...
        success, output = self.run_command(
            ["dotnet", "test", "/p:CollectCoverage=true"], "dotnet test"
        )
    ...
    def check_typescript(self):
        ...
        # TypeScript compiler
        print("Running tsc...")
        success, output = self.run_command(["npx", "tsc", "--noEmit"], "tsc")
        self.results.append(("TypeScript compiler", success, output))

        # Jest tests
        print("Running jest...")
        success, output = self.run_command(["npm", "test", "--", "--coverage"], "jest")
    ...
    def detect_project_type(self) -> List[str]:
        ...
        # Check for C#
        if list(self.project_root.glob("*.sln")) or list(
            self.project_root.glob("**/*.csproj")
        ):
...
def main():
    ...
    if not project_types:
        ...
        print(
            "Looking for: Python (setup.py, pyproject.toml), C# (*.sln, *.csproj), TypeScript/Node.js (package.json)"
        )
        sys.exit(1)
