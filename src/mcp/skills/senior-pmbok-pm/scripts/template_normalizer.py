...
def analyze_template(text: str) -> Dict[str, object]:
    ...
    has_purpose = any(
        "propósito" in line.lower() or "purpose" in line.lower() for line in lines[:20]
    )
    has_doc_map = any(
        "DOCUMENTACAO" in line or "DOCUMENTAÇÃO" in line for line in lines
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate and normalize TEMPLATE.md files."
    )
    ...
    parser.add_argument(
        "--report-output", help="Write report to a file instead of stdout"
    )
...
def main() -> int:
    ...
    if args.apply:
        output_path = (
            Path(args.output)
            if args.output
            else template_path.with_suffix(".normalized.md")
        )
