...
def audit_quality(text: str) -> Dict[str, object]:
    ...
    checks = {
        "has_version_control": any(
            token in lower for token in ["versão", "version", "histórico", "history"]
        ),
        "has_ownership": any(
            token in lower
            for token in ["responsável", "owner", "aprovador", "approver"]
        ),
        "has_traceability": any(
            token in lower for token in ["referência", "reference", "rastro", "trace"]
        ),
        "has_placeholders": bool(extract_placeholders(text)),
    }
    score = sum(
        1 for key, value in checks.items() if key != "has_placeholders" and value
    )
    total = len(checks) - 1
    return {
        "checks": checks,
