...
class WorkflowExporter:
    ...
    def to_plantuml_activity(workflow_data: Dict[str, Any]) -> str:
        ...
        lines = ["@startuml", f"title {command.title()} Workflow", "start"]
    ...
    def to_mermaid_flowchart(workflow_data: Dict[str, Any]) -> str:
        ...
        lines = ["flowchart TD", f"    Start([Start: {command.title()} Workflow])"]
        ...
        if isinstance(artifacts_dict, dict):
            ...
            for artifact_name, details in artifacts_dict.items():
                if isinstance(details, dict):
                    ...
                    lines.append(
                        f"  | {artifact_name} | {status} | {version} | {owner} | {issues}"
                    )
                else:
        ...
        if isinstance(audit_data, dict):
            for artifact, details in audit_data.items():
                ...
                if isinstance(details, dict):
                    ...
                    detail_str = ", ".join(
                        f"{k}={v}" for k, v in details.items() if k != "status"
                    )
                else:
                    status = "N/A"
