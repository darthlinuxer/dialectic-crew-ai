...
def print_report(report: IntrospectionReport, max_items: int) -> None:
    ...
    for index, opportunity in enumerate(report.opportunities):
        ...
        print(f"  {marker} [{opportunity.estimated_impact.upper():>6}] {opportunity.id}: {opportunity.title}")
...
def build_pr_body(
    ...
    for opportunity in selected:
        lines.append(f"- **{opportunity.title}** ({opportunity.category}, {opportunity.estimated_impact})")
